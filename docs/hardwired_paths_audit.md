...
| `ui_qt/forms/visualizations.py:1233` (per-route src_dir) | ✓ Fixed 122dc | Added `_resolve_viz_source_dir` helper + `_VIZ_SOURCE_V1_MAP` dict that maps each route's `data_*_source` value to the v1 directory under `derived/` (with latest-run-or-parent semantics matching ConfigReader). All 4 actually-used route sources have explicit v1 mappings; the helper falls back to `derived/<name>/` for unknown sources. |
