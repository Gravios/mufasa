...
def get_nose_tail_from_vertices(vertices: np.ndarray,
    ...
    .. seealso::
       For computing left and right points using the nose and tail, see :func:`~mufasa.data_processors.find_animal_blob_location.get_left_right_points`.
       For blob tracking that uses this function, see :func:`~mufasa.video_processors.blob_tracking_executor.BlobTrackingExecutor`.
...
def get_left_right_points(hull_vertices: np.ndarray,
    ...
    .. seealso::
       For computing anterior and posterior points, see :func:`~mufasa.data_processors.find_animal_blob_location.get_nose_tail_from_vertices`.
       For blob tracking that uses this function, see :func:`~mufasa.video_processors.blob_tracking_executor.BlobTrackingExecutor`.
...
def get_blob_vertices_from_imgs(frm_idxs: Tuple[int, List[int]],
    ...
    .. seealso::
       To create background subtracted videos, use e.g., :func:`mufasa.video_processors.video_processing.video_bg_subtraction_mp`, or :func:`~mufasa.video_processors.video_processing.video_bg_subtraction`.
       To get ``img`` dict, use :func:`~mufasa.utils.read_write.read_img_batch_from_video_gpu` or :func:`~mufasa.mixins.image_mixin.ImageMixin.read_img_batch_from_video`.
       For relevant notebook, see `BACKGROUND REMOVAL <https://mufasa-uw-tf-dev.readthedocs.io/en/latest/nb/bg_remove.html>`__.
...
def get_blob_vertices_from_video(video_path: Union[str, os.PathLike],
    ...
    .. seealso::
       For visualization of results, see :func:`mufasa.plotting.blob_plotter.BlobPlotter` and :func:`mufasa.mixins.plotting_mixin.PlottingMixin._plot_blobs`
       Background subtraction can be performed using :func:`~mufasa.video_processors.video_processing.video_bg_subtraction_mp` or :func:`~mufasa.video_processors.video_processing.video_bg_subtraction`.

    .. note::
       If ``inclusion_zone`` is not None, then the largest blob will be searched for **inside** the passed vertices.

    :param Union[str, os.PathLike] video_path: Path to the video file from which to extract frames. Often, a background subtracted video, which can be created with e.g., :func:`mufasa.video_processors.video_processing.video_bg_subtraction_mp`.
