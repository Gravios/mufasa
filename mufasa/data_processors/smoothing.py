...
class Smoothing(ConfigReader):
    ...
    :examples:
    >>> smoother = Smoothing(data_path='/path/to/mufasa/troubleshooting/two_black_animals_14bp/project_folder/csv/input_csv/Together_1.csv', config_path=r'/path/to/mufasa/troubleshooting/two_black_animals_14bp/project.toml', method='Savitzky-Golay', time_window=500, multi_index_df_headers=True, copy_originals=True)
    >>> smoother.run()
    """
    ...
    def __init__(self,
        ...
        # Audit A1 fix: auto-detect multi-index headers when not explicitly
        # passed. Previously this was the popup's responsibility, and the
        # Smoothing popup got it wrong (raw string equality; see
        # mufasa/ui/pop_ups/smoothing_popup.py:82 in the Tk original)
        # while InterpolatePopUp got it right (Path.resolve()). Centralising
        # here fixes both at once.
        if multi_index_df_headers is None:
            from pathlib import Path as _P
            raw_dir = _P(self.input_csv_dir).resolve()
