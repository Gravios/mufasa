...
class CueLightDataAnalyzerPopUp(ConfigReader, PopUpMixin):
    ...
    def __init__(self,
        ...
        if len(missing_cue_lights) > 0: raise NoROIDataError(msg=f'{len(missing_cue_lights)} cue-lights are not drawn in the Mufasa project. The {missing_cue_lights} ROIs cannot be found in the {self.roi_coordinates_path} file.', source=self.__class__.__name__)
