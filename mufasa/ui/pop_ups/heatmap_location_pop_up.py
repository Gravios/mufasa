...
from mufasa.plotting.heat_mapper_location import HeatmapperLocationSingleCore
from mufasa.plotting.heat_mapper_location_mp import \
...
from mufasa.utils.enums import Formats, Links, Paths
from mufasa.utils.lookups import get_named_colors
from mufasa.utils.read_write import get_file_name_info_in_directory, str_2_bool
...
class HeatmapLocationPopup(PopUpMixin, ConfigReader):
    ...
    def __init__(self,
        ...
        self.palette_dropdown = MufasaDropDown(parent=self.style_settings_frm, dropdown_options=self.palette_options, label='PALETTE: ', label_width=25, dropdown_width=30, value='jet', img='color_wheel', tooltip_key='HEATMAP_LOCATION_PALETTE')
        self.shading_dropdown = MufasaDropDown(parent=self.style_settings_frm, dropdown_options=self.shading_options, label='SHADING: ', label_width=25, dropdown_width=30, value=self.shading_options[1], img='shade', tooltip_key='HEATMAP_LOCATION_SHADING')
        self.bp_dropdown = MufasaDropDown(parent=self.style_settings_frm, dropdown_options=self.body_parts_lst, label='BODY-PART: ', label_width=25, dropdown_width=30, value=self.body_parts_lst[0],img='pose', tooltip_key='HEATMAP_LOCATION_BODY_PART')
        self.max_time_scale_dropdown = MufasaDropDown(parent=self.style_settings_frm, dropdown_options=max_scales, label='MAX TIME SCALE (S): ', label_width=25, dropdown_width=30, value=max_scales[0], img='timer_2', tooltip_key='HEATMAP_LOCATION_MAX_TIME_SCALE')
        self.bin_size_dropdown = MufasaDropDown(parent=self.style_settings_frm, dropdown_options=self.heatmap_bin_size_options, label='BIN SIZE (MM): ', label_width=25, dropdown_width=30, value="20×20", img='rectangle_red', tooltip_key='HEATMAP_LOCATION_BIN_SIZE')
        self.legend_dropdown = MufasaDropDown(parent=self.style_settings_frm, dropdown_options=['TRUE', 'FALSE'], label='SHOW TIME COLOR LEGEND: ', label_width=25, dropdown_width=30, value='TRUE', img='palette_small', tooltip_key='HEATMAP_LOCATION_SHOW_LEGEND')
        self.min_seconds_dropdown = MufasaDropDown(parent=self.style_settings_frm, dropdown_options=min_seconds_options, label='MINIMUM SECONDS: ', label_width=25, dropdown_width=30, value='NONE', img='timer', tooltip_key='HEATMAP_LOCATION_MIN_SECONDS')
        self.line_dropdown = MufasaDropDown(parent=self.style_settings_frm, dropdown_options=line_color_options, label='LINE COLOR: ', label_width=25, dropdown_width=30, value='white', img='line', tooltip_key='HEATMAP_LOCATION_LINE_COLOR')
        ...
        self.multiprocess_dropdown = MufasaDropDown(parent=self.settings_frm, dropdown_options=list(range(2, self.cpu_cnt)), label='CPU CORES: ', label_width=25, dropdown_width=30, value=int(self.cpu_cnt/3), img='cpu_small', tooltip_key='HEATMAP_LOCATION_CPU_CORES')
        ...
        self.bg_dropdown = MufasaDropDown(parent=self.bg_frm, dropdown_options=HEATMAP_BG_OPTIONS, label='HEATMAP BACKGROUND: ', label_width=25, dropdown_width=35, value='NONE', img='background', command=self._set_select_bg_frm, tooltip_key='HEATMAP_CLF_BACKGROUND')
        ...
        self.single_video_dropdown = MufasaDropDown(parent=self.run_single_video_frm, label="VIDEO:", dropdown_options=list(self.files_found_dict.keys()), label_width=12, dropdown_width=40, value=list(self.files_found_dict.keys())[0], tooltip_key='HEATMAP_LOCATION_SINGLE_VIDEO')
    ...
    def _set_select_bg_frm(self, selection):
        ...
        if selection == VIDEO:
            self.opacity_dropdown = MufasaDropDown(parent=self.bg_frm, dropdown_options=list(np.arange(5, 105, 5)), label='HEATMAP OPACITY (%): ', label_width=25, dropdown_width=35, value=50, img='opacity', tooltip_key='HEATMAP_CLF_OPACITY')
            self.keypoint_dropdown = MufasaDropDown(parent=self.bg_frm, dropdown_options=['TRUE', 'FALSE'], label='SHOW KEYPOINT: ', label_width=25, dropdown_width=30, value='TRUE', img='pose', tooltip_key='HEATMAP_CLF_SHOW_KEYPOINT')
            self.opacity_dropdown.grid(row=1, sticky=NW)
            self.keypoint_dropdown.grid(row=2, sticky=NW)
        elif selection == VIDEO_FRM:
            self.opacity_dropdown = MufasaDropDown(parent=self.bg_frm, dropdown_options=list(np.arange(5, 105, 5)), label='HEATMAP OPACITY (%): ', label_width=25, dropdown_width=30, value=50, img='opacity', tooltip_key='HEATMAP_CLF_OPACITY')
