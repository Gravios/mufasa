...
from mufasa.roi_tools.roi_aggregate_stats_mp import \
...
class ROIAggregateDataAnalyzerPopUp(PopUpMixin, ConfigReader):
    ...
    def __init__(self, config_path: Union[str, os.PathLike]):
        ...
        self.animal_cnt_dropdown = MufasaDropDown(parent=self.animal_cnt_frm, label="# OF ANIMALS", dropdown_options=list(range(1, self.animal_cnt + 1)), label_width=30, dropdown_width=30, value=1, command= lambda x: self._get_settings_frm(x), img='abacus', tooltip_key='ROI_TRACKING_NUMBER_OF_ANIMALS')
        ...
        self.core_cnt_dropdown = MufasaDropDown(parent=self.core_cnt_frm, label="# CPU CORES", dropdown_options=list(range(1, find_core_cnt()[0])), label_width=30, dropdown_width=30, value=1, tooltip_key='CPU_ROI_DESCRIPTIVE_ANALYSIS', img='cpu_small')
    ...
    def _get_settings_frm(self, bp_cnt):
        ...
        for bp_cnt in range(int(self.animal_cnt_dropdown.getChoices())):
            self.body_parts_dropdowns[bp_cnt] = MufasaDropDown(parent=self.body_part_frm, label=f"BODY-PART {str(bp_cnt + 1)}:", dropdown_options=self.body_parts_lst, label_width=30, value=self.body_parts_lst[bp_cnt], dropdown_width=30, img='mouse_small', tooltip_key='ROI_TRACKING_BODY_PART')
