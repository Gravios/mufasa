...
ICON_WINDOWS = os.path.join(os.path.dirname(mufasa.__file__), Paths.LOGO_ICON_WINDOWS_PATH.value)
ICON_DARWIN = os.path.join(os.path.dirname(mufasa.__file__), Paths.LOGO_ICON_DARWIN_PATH.value)
...
class InteractiveProbabilityGrapher(ConfigReader):
    ...
    .. note::
       `Validation tutorial <https://github.com/sgoldenlab/mufasa/blob/master/docs/Scenario1.md#critical-validation-step-before-running-machine-model-on-new-data>`__.
    ...
    :example:
        >>> interactive_plotter = InteractiveProbabilityGrapher(config_path='project.toml', file_path='features.csv', model_path='classifier.sav')
