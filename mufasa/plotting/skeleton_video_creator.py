...
from mufasa.utils.data import (
    ...
    egocentrically_align_pose_numba,
    get_cpu_pool,
    terminate_cpu_pool,
)
...
class SkeletonVideoCreator:
    ...
    * **Center anchor** (``anchor_bp`` only, no egocentric anchors): each frame, shifts all
    ...
    def __init__(self,
                 ...
                 video_info_path: str | os.PathLike | None = None,
                 resolution: tuple[int, int] = (500, 500),
                 bg_color: tuple[int, int, int] = (0, 0, 0),
                 anchor_bp: str | None = None,
                 skeleton: list[tuple[str, str]] | None = None,
                 circle_size: int | None = None,
                 line_thickness: int | None = None,
                 ego_anchor_1: str | None = None,
                 ego_anchor_2: str | None = None,
                 ego_direction: int = 0,
                 omit_bps: list[str] | None = None,
                 palette: str = 'Set1',
                 bp_threshold: float = 0.0,
                 core_cnt: int = -1,
                 verbose: bool = True):
