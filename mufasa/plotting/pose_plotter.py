    ...
    def __init__(self,
        ...
        if palettes is not None:
            ...
            if len(list(palettes.keys())) != self.config.animal_cnt:
                raise CountError(msg=f'The number of color palettes ({(len(list(palettes.keys())))}) is not the same as the number of animals ({(self.config.animal_cnt)}) in the Mufasa project at {self.config.project_path}')
