...
class FeatureExtractionSupplemental(FeatureExtractionMixin):
    """
    Additional feature extraction method not called by default feature extraction classes from ``mufasa.feature_extractors``.
    ...
    def _helper_euclidean_distance_timeseries_change(
        ...
        """
        Private jitted helper called by ``mufasa.mixins.feature_extraction_supplemental_mixin.FeatureExtractionSupplemental.euclidean_distance_timeseries_change``
        """
    ...
    def rolling_peak_count_ratio(data: np.ndarray, time_windows: np.ndarray, fps: int) -> np.ndarray:
        ...
        .. seealso::
           For single time-series analysis, see :func:`mufasa.mixins.feature_extraction_supplement_mixin.FeatureExtractionSupplemental.peak_ratio`
    ...
    def sequential_lag_analysis(data: pd.DataFrame, criterion: str, target: str, time_window: float, fps: float) -> float:
        ...
        .. seealso::
           For altenative method, see :func:`~mufasa.data_processorsfsttc_calculator.FSTTCCalculator`
    ...
    def distance_and_velocity(x: np.ndarray,
        ...
        :param x: Array containing movement data. For example, created by ``mufasa.mixins.FeatureExtractionMixin.framewise_euclidean_distance``. If its a 2-dimensional array, then we assume its pixel coordinates. If it's a 1d array, we assume its frame-wise euclidean distances.
    ...
    def movement_stats_from_bouts_df(bp_data: np.ndarray,
        ...
        .. seealso::
           To compute ``bout_df``, use :func:`mufasa.utils.data.detect_bouts`
        ...
        :param pd.DataFrame bout_df: Dataframe with detected events. Returned by :func:`mufasa.utils.data.detect_bouts`.
