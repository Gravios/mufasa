...
class CustomFeatureExtractor(ConfigReader):
    ...
    .. note::
        ...
        If the feature extraction class relies on argparse AND inherits ``mufasa.mixins.abstract_classes.AbstractFeatureExtraction``
        ...
        Thus, it is recommended that the custom feature extraction class inherits ``mufasa.mixins.abstract_classes.AbstractFeatureExtraction`` and is executed through argparse.

        For an example feature extraction class that inherits from ``mufasa.mixins.abstract_classes.AbstractFeatureExtraction`` and is executed through argparse, see
        `this file <https://github.com/sgoldenlab/mufasa/blob/master/misc/geometry_feature_extraction.py>`_. For an example feature extraction class that does **NOT** inherit from ``mufasa.mixins.abstract_classes.AbstractFeatureExtraction``
        and does not use argparse, see `this file <https://github.com/lapphe/AMBER-pipeline/blob/main/SimBA_AMBER_project/AMBER_2_0__feature_extraction/amber_feature_extraction_20230815.py>`_. More examples can be found in `this github documentation <https://github.com/sgoldenlab/mufasa/blob/master/docs/extractFeatures.md>`_.
