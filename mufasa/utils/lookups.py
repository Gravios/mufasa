...
from mufasa.utils.enums import OS, UML, Defaults, Formats, Keys, Methods, Options, Paths
...
def get_bp_config_code_class_pairs() -> dict[str, object]:
    ...
    from mufasa.feature_extractors.amber_feature_extractor import AmberFeatureExtractor
    ...
    from mufasa.feature_extractors.feature_extractor_8bps_2_animals import (
...
def get_display_resolution() -> tuple[int, int]:
    ...
    .. note::
       May return the virtual geometry in multi-display setups. To return the resolution of each available monitor in mosaic, see :func:`mufasa.utils.lookups.get_monitor_info`.
...
def get_table(data: dict[str, Any],
    ...
    """
     ...
     :param Literal["grid"] tablefmt: Table format style. For options, see mufasa.utils.enums.Formats.VALID_TABLEFMT
...
def print_video_meta_data(data_path: str | os.PathLike) -> None:
    ...
    .. seealso::
       To get video metadata as a dictionary without printing, use :func:`mufasa.utils.read_write.get_video_meta_data`.
       To get video metadata as a table without printing, use :func:`mufasa.utils.lookups.get_table`.
    ...
    for animal_name, animal_bps in results.items():
        for bp_name, bp_values in animal_bps.items():
           if len(bp_values.keys()) == 0:
               raise InvalidInputError(msg=f'Could not detect a body-part for animal {animal_name}, body-part {bp_name} in Mufasa project. Make sure the body-part configuration file at {Paths.BP_NAMES.value} lists the appropriate body-parts', source=create_directionality_cords.__name__)
...
def check_for_updates(time_out: int = 2):
    ...
    if latest_simba_version == env_simba_version:
        ...
        msg = (f'NEW SimBA VERSION AVAILABLE. \nYou have SimBA version {env_simba_version}. \nThe latest version is {latest_simba_version}. '
               f'\nYou can update using "pip install mufasa-uw-tf-dev --upgrade"')
