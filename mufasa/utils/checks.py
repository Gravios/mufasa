...
def check_that_column_exist(df: pd.DataFrame,
    ...
    .. seealso::
       Consider :func:`mufasa.utils.checks.check_valid_dataframe` instead.
...
def check_if_valid_input(
    ...
    .. seealso::
       Consider :func:`mufasa.utils.checks.check_str`.
...
def check_ffmpeg_available(raise_error: Optional[bool] = False) -> Union[bool, None]:
    ...
    .. seealso::
       To check which encoders are available in FFMpeg installation, see :func:`mufasa.utils.lookups.get_ffmpeg_encoders`
...
def check_if_module_has_import(parsed_file: ast.Module, import_name: str) -> bool:
    ...
    Used for e.g., user custom feature extraction classes in ``mufasa.utils.custom_feature_extractor.CustomFeatureExtractor``.
    ...
    :example:
    >>> parsed_file = ast.parse(Path('/mufasa/misc/piotr.py').read_text())
...
def check_if_keys_exist_in_dict(
    ...
    .. seealso::
       Consider :func:`mufasa.utils.checks.check_valid_dict`
...
def check_video_has_rois(roi_dict: Dict[str, pd.DataFrame],
    ...
    .. note::
       To get roi dictionary, see :func:`mufasa.mixins.config_reader.ConfigReader.read_roi_data`.
...
def is_video_color(video: Union[str, os.PathLike, cv2.VideoCapture]) -> bool:
    ...
    .. seealso::
       :func:`mufasa.mixins.image_mixin.ImageMixin.is_video_color`
...
def is_lxc_container() -> bool:
    ...
    .. note::
       See GitHub issue 457 for origin - https://github.com/sgoldenlab/mufasa/issues/457#issuecomment-3052631284
...
def check_valid_codec(codec: str, raise_error: bool = True, source: str = ''):
    ...
    .. seealso::
       To get a list of all available encoders, see :func:`~mufasa.utils.lookups.get_ffmpeg_encoders`.
       To check if FFmpeg is available, see :func:`~mufasa.utils.checks.check_ffmpeg_available`.
    ...
    """
    from mufasa.utils.lookups import get_ffmpeg_encoders; encoders = get_ffmpeg_encoders()
