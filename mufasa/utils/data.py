...
def detect_bouts(data_df: pd.DataFrame, target_lst: Union[List[str], str], fps: Union[int, float]) -> pd.DataFrame:
    ...
    .. seealso::
       For multi-class Boolean classifiers, see :func:`mufasa.utils.data.detect_bouts_multiclass`.
...
def detect_bouts_multiclass(data: pd.DataFrame, target: str, fps: int = 1, classifier_map: Dict[int, str] = None) -> pd.DataFrame:
    ...
    .. seealso::
       For single class Boolean classifiers, see :func:`mufasa.utils.data.detect_bouts`.
...
def smooth_data_savitzky_golay(
    ...
    .. important::
       LEGACY: USE ``mufasa.utils.data.savgol_smoother`` instead.
...
def find_frame_numbers_from_time_stamp(start_time: str, end_time: str, fps: int) -> List[int]:
    ...
    .. note::
       For the converse (find frame numbers from start and in HH:MM:SS format), use func:`mufasa.utils.read_write.find_time_stamp_from_frame_numbers`.
...
def freedman_diaconis(data: np.ndarray) -> Tuple[float, int]:
    ...
    .. note::
       Can also use ``mufasa.utils.data.bucket_data`` passing method ``fd``.
...
def bucket_data(data: np.ndarray, method: Literal["fd", "doane", "auto", "scott", "stone", "rice", "sturges", "sqrt"] = "auto",) -> Tuple[float, int]:
    ...
    .. seealso::
       :func:`mufasa.utils.data.freedman_diaconis`
       :func:`mufasa.utils.data.bucket_data`
...
def bucket_data_mp(data: np.ndarray,
    ...
    .. seealso::
       :func:`mufasa.utils.data.freedman_diaconis`
       :func:`mufasa.utils.data.bucket_data_mp`
...
def fast_minimum_rank(data: np.ndarray, descending: Optional[bool] = True) -> np.ndarray:
    ...
    .. seealso::
       :func:`mufasa.utils.data.fast_mean_rank`
...
def fast_mean_rank(data: np.ndarray, descending: Optional[bool] = True) -> np.ndarray:
    ...
    .. seealso::
       :func:`mufasa.utils.data.fast_minimum_rank`
...
def run_user_defined_feature_extraction_class(file_path: Union[str, os.PathLike], config_path: Union[str, os.PathLike]) -> None:
    ...
    .. warning::
       ...
       Legacy function. The GUI since 12/23 uses :func:`mufasa.utils.custom_feature_extractor.UserDefinedFeatureExtractor`.

    .. note::
       `Tutorial <https://github.com/sgoldenlab/mufasa/blob/master/docs/extractFeatures.md>`_.
...
def savgol_smoother(data: Union[pd.DataFrame, np.ndarray],
    ...
    .. seealso::
       For 'bartlett', 'blackman', 'boxcar', 'cosine', 'gaussian', 'hamming', 'exponential' smoothing, see func:`mufasa.utils.data.df_smoother`.
       For low-pass Fourier smoothing, see :func:`mufasa.utils.data.fft_lowpass_filter`.
...
def df_smoother(data: pd.DataFrame,
    ...
    .. seealso::
       For low-pass Fourier smoothing, see :func:`mufasa.utils.data.fft_lowpass_filter`.
       For Savitzky-Golay smoothing, see :func:`mufasa.utils.data.savgol_smoother`.
...
def egocentrically_align_pose(data: np.ndarray,
    ...
    .. seealso::
       For numba acceleration, see :func:`mufasa.utils.data.egocentrically_align_pose_numba`.
       To align both pose and video, see :func:`mufasa.data_processors.egocentric_aligner.EgocentricalAligner`.
       To egocentrically rotate video, see :func:`mufasa.video_processors.egocentric_video_rotator.EgocentricVideoRotator`
...
def egocentrically_align_pose_numba(data: np.ndarray,
    ...
    .. seealso::
       For numpy function, see :func:`mufasa.utils.data.egocentrically_align_pose`.
       To align both pose and video, see :func:`mufasa.data_processors.egocentric_aligner.EgocentricalAligner`.
       To egocentrically rotate video, see :func:`mufasa.video_processors.egocentric_video_rotator.EgocentricVideoRotator`
...
def center_rotation_warpaffine_vectors(rotation_vectors: np.ndarray, centers: np.ndarray):
    ...
    .. note::
       `rotation_vectors` and `centers` are returned by :func:`mufasa.utils.data.egocentrically_align_pose`, or :func:`mufasa.utils.data.egocentrically_align_pose_numba`
       `results are used within e.g., :func:`mufasa.video_processors.egocentric_video_rotator.EgocentricVideoRotator`
...
def align_target_warpaffine_vectors(centers: np.ndarray, target: np.ndarray):
    ...
    .. note::
       `centers` are returned by :func:`mufasa.utils.data.egocentrically_align_pose`, or :func:`mufasa.utils.data.egocentrically_align_pose_numba`
...
def _bilinear_interpolate(image: np.ndarray, x: int, y: int):
    """
    Helper called by :func:`mufasa.sandbox.warp_numba.egocentric_frm_rotator` Perform bilinear interpolation on an image at fractional coordinates (x, y). Assumes coordinates (x, y) are within the bounds of the image.
    """
...
def egocentric_frm_rotator(frames: np.ndarray,
    ...
    .. note::
       To create rotation matrices, see :func:`mufasa.utils.data.center_rotation_warpaffine_vectors` and :func:`mufasa.utils.data.align_target_warpaffine_vectors`
