...
def get_video_meta_data(video_path: Union[str, os.PathLike, cv2.VideoCapture],
    ...
    .. seealso::
       To use FFmpeg instead of OpenCV, see :func:`mufasa.utils.read_write.get_video_info_ffmpeg`.
...
def get_video_info_ffmpeg(video_path: Union[str, os.PathLike]) -> Dict[str, Any]:
    ...
    .. seealso::
       To use OpenCV instead of FFmpeg, see :func:`mufasa.utils.read_write.get_video_meta_data`
...
def read_video_info(video_name: str,
    ...
    :param pd.DataFrame vid_info_df: Parsed ``project_folder/logs/video_info.csv`` file. This file can be parsed by :func:`mufasa.utils.read_write.read_video_info_csv`.
...
def read_frm_of_video(video_path: Union[str, os.PathLike, cv2.VideoCapture],
    ...
    .. seealso::
       To read a batch of images with GPU acceleration, see :func:`mufasa.utils.read_write.read_img_batch_from_video_gpu`.
       To read a batch of videos using multicore CPU acceleration, see :func:`mufasa.utils.read_write.read_img_batch_from_video`.
       To read frames batches asynchronously, see :func:`mufasa.video_processors.async_frame_reader.AsyncVideoFrameReader`.
...
def get_pkg_version(pkg: str, raise_error: Optional[bool] = False):
    ...
    :example:
    >>> get_pkg_version(pkg='mufasa-uw-tf-dev')
    >>> 1.82.7
...
def read_img_batch_from_video_gpu(video_path: Union[str, os.PathLike],
    ...
    .. seealso::
       For CPU multicore acceleration, see :func:`mufasa.mixins.image_mixin.ImageMixin.read_img_batch_from_video` or :func:`mufasa.utils.read_write.read_img_batch_from_video`.
...
def get_env_pose_config_dir(raise_error: Optional[bool] = True):
    ...
    pose_config_dir = os.path.join(site_packages_dir, 'mufasa', 'pose_configurations')
    if os.path.isdir(pose_config_dir):
        ...
        if raise_error:
            raise MufasaPackageVersionError(msg=f'pose_configurations directory could not be found. Expected directory: {pose_config_dir}', source=get_env_pose_config_dir.__name__)
        return None
