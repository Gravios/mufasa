...
class YoloNVDECInference(object):
    ...
    .. important::
       ...
       :func:`~mufasa.utils.lookups.get_nvdec_count`. If your GPU is not listed or the count is incorrect, pass ``max_workers`` explicitly.
    ...
    .. seealso::
       * :class:`~mufasa.third_party_label_appenders.transform.sam3_to_yolo_bbox.SAM3ToYoloBBox` — create a YOLO bounding-box project from SAM3 annotations.
       * :class:`~mufasa.third_party_label_appenders.transform.sam3_to_yolo_seg.SAM3ToYoloSeg` — create a YOLO segmentation project from SAM3 annotations.
       * :class:`~mufasa.model.yolo_inference.YoloInference` — CPU-based YOLO bounding-box inference.
       * :class:`~mufasa.model.yolo_pose_inference.YOLOPoseInference` — CPU-based YOLO pose inference.
       * :class:`~mufasa.model.yolo_seg_inference.YOLOSegmentationInference` — CPU-based YOLO segmentation inference.
    ...
    :param Union[str, os.PathLike] engine_path: Path to TensorRT engine file (.engine). If alternative model file exists, convert it to engine using :func:`mufasa.utils.yolo.export_yolo_model`. For multi-GPU, place the source ``.pt`` weights alongside the engine — per-GPU engines are auto-exported on first run.
    ...
    def __init__(self,
        ...
        for gid in gpu_ids[1:]:
            ...
            if not os.path.isfile(gpu_engine):
                from mufasa.utils.yolo import export_yolo_model
