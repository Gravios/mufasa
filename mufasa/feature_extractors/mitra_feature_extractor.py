...
from mufasa.data_processors import circular_dispatch as _cd
from mufasa.feature_extractors.perimeter_jit import jitted_hull
from mufasa.mixins.abstract_classes import AbstractFeatureExtraction
...
class MitraFeatureExtractor(ConfigReader,
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.outlier_corrected_paths):
            ...
            # The three functions below route through ``circular_dispatch``:
            # auto-GPU on machines with CUDA (RTX 5070 Ti etc.), CPU otherwise.
            # Expected speedup at N≥1M: ~15-50× — see
            # scripts/benchmark_circular_stats.py. Call-sites below that
            # still call ``CircularStatisticsMixin.*`` directly (rao_spacing,
            # circular_range, angular_diff, circular_correlation) don't yet
            # have GPU implementations.
            results['CIRCULAR_INSTANTANEOUS_ANGULAR_VELOCITY'] = _cd.instantaneous_angular_velocity(data=direction_degrees, bin_size=1)
            ...
            circular_std = pd.DataFrame(_cd.sliding_circular_std(data=direction_degrees, time_windows=TIME_WINDOWS, fps=int(fps)), columns=['CIRCULAR_HEAD_DIRECTION_STD_250', 'CIRCULAR_HEAD_DIRECTION_STD_500', 'CIRCULAR_HEAD_DIRECTION_STD_1000', 'CIRCULAR_HEAD_DIRECTION_STD_2000'])
            ...
            mean_resultant_vector_length = pd.DataFrame(_cd.sliding_mean_resultant_vector_length(data=direction_degrees, fps=int(fps), time_windows=TIME_WINDOWS), columns=['CIRCULAR_MEAN_RESULTANT_LENGTH_250', 'CIRCULAR_MEAN_RESULTANT_LENGTH_500', 'CIRCULAR_MEAN_RESULTANT_LENGTH_1000', 'CIRCULAR_MEAN_RESULTANT_LENGTH_2000'])
            ...
            sliding_z_p_low = pd.DataFrame(Statistics.sliding_z_scores(data=p_df['PROBABILITIES_LOW_COUNT'].values.astype(np.float32), time_windows=TIME_WINDOWS, fps=int(fps)), columns=['PROBABILITIES_LOW_COUNT_SLIDING_Z_SCORE_250', 'PROBABILITIES_LOW_COUNT_SLIDING_Z_SCORE_500', 'PROBABILITIES_LOW_COUNT_SLIDING_Z_SCORE_1000', 'PROBABILITIES_LOW_COUNT_SLIDING_Z_SCORE_2000'])
            sliding_z_p_high = pd.DataFrame(Statistics.sliding_z_scores(data=p_df['PROBABILITIES_HIGH_COUNT'].values.astype(np.float32), time_windows=TIME_WINDOWS, fps=int(fps)), columns=['PROBABILITIES_HIGH_COUNT_SLIDING_Z_SCORE_250', 'PROBABILITIES_HIGH_COUNT_SLIDING_Z_SCORE_500', 'PROBABILITIES_HIGH_COUNT_SLIDING_Z_SCORE_1000', 'PROBABILITIES_HIGH_COUNT_SLIDING_Z_SCORE_2000'])
