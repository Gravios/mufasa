...
class ExtractFeaturesFrom8bps(ConfigReader, FeatureExtractionMixin):
    ...
    .. note::
       * `Feature extraction tutorial <https://github.com/sgoldenlab/mufasa/blob/master/docs/tutorial.md#step-5-extract-features>`_.
       * `Expected pose configuration <https://github.com/sgoldenlab/mufasa/blob/master/mufasa/pose_configurations/schematics/3.png>`_
    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            for window in self.roll_windows_values:
                col_name = f"Total_movement_M1_median_{str(window)}"
                self.out_data[col_name] = (
                    self.out_data["Total_movement_all_bodyparts_M1"]
                    .rolling(int(window), min_periods=1)
                    .median()
                )
                col_name = f"Total_movement_M1_mean_{str(window)}"
                self.out_data[col_name] = (
                    self.out_data["Total_movement_all_bodyparts_M1"]
                    .rolling(int(window), min_periods=1)
                    .mean()
                )
                col_name = f"Total_movement_M1_sum_{str(window)}"
                self.out_data[col_name] = (
                    self.out_data["Total_movement_all_bodyparts_M1"]
                    .rolling(int(window), min_periods=1)
                    .sum()
