...
from mufasa.utils.checks import (check_file_exist_and_readable,
                                ...
                                check_valid_tuple)
from mufasa.utils.data import (align_target_warpaffine_vectors,
...
from mufasa.utils.read_write import (concatenate_videos_in_folder,
                                    ...
                                    get_fn_ext, get_video_meta_data, read_frm_of_video,
...
class EgocentricVideoRotator:
    ...
    .. seealso::
       To perform joint egocentric alignment of both pose and video, or pose only, use :func:`~mufasa.data_processors.egocentric_aligner.EgocentricalAligner`.
       To produce rotation vectors, use :func:`~mufasa.utils.data.egocentrically_align_pose_numba` or :func:`~mufasa.utils.data.egocentrically_align_pose`.

    :param Union[str, os.PathLike] video_path: Path to a video file.
    :param np.ndarray centers: A 2D array of shape `(num_frames, 2)` containing the original locations of `anchor_1_idx` in each frame before alignment. Returned by :func:`~mufasa.utils.data.egocentrically_align_pose_numba` or :func:`~mufasa.utils.data.egocentrically_align_pose`.
    :param np.ndarray rotation_vectors: A 3D array of shape `(num_frames, 2, 2)` containing the rotation matrices applied to each frame. Returned by :func:`~mufasa.utils.data.egocentrically_align_pose_numba` or :func:`~mufasa.utils.data.egocentrically_align_pose`.
